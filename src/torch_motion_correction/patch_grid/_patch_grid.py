...
def patch_grid_generator(
    images: torch.Tensor,
    patch_shape: tuple[int, int] | tuple[int, int, int],
    patch_step: tuple[int, int] | tuple[int, int, int],
    distribute_patches: bool = True,
) -> Tuple[Iterator[torch.Tensor], torch.Tensor, Tuple[int, ...]]:
    """Memory-efficient patch extraction using a generator.
    
    Parameters
    ----------
    images: torch.Tensor
        Input images to extract patches from.
    patch_shape: tuple[int, int] | tuple[int, int, int]
    ...
    Returns
    -------
    patch_generator: Iterator[torch.Tensor]
        Generator that yields individual patches.
    patch_centers: torch.Tensor
        Centers of all patches.
    patch_grid_shape: Tuple[int, ...]
        Shape of the patch grid (number of patches in each dimension).
    """
    if len(patch_shape) != len(patch_step):
        raise ValueError('patch shape and step must have the same number of dimensions.')
    ...
    ndim = len(patch_shape)
    if ndim == 2:
        return _patch_grid_generator_2d(
            images=images,
            patch_shape=patch_shape,
            patch_step=patch_step,
            distribute_patches=distribute_patches,
        ...
        return _patch_grid_generator_3d(
            images=images,
            patch_shape=patch_shape,
            patch_step=patch_step,
            distribute_patches=distribute_patches,
...
def _patch_grid_generator_2d(
    images: torch.Tensor,
    patch_shape: tuple[int, int],
    patch_step: tuple[int, int],
    distribute_patches: bool = True,
) -> Tuple[Iterator[torch.Tensor], torch.Tensor, Tuple[int, int]]:
    """Generate 2D patches on-demand to save memory."""
    patch_centers = patch_grid_centers(
        image_shape=images.shape[-2:],
        patch_shape=patch_shape,
        patch_step=patch_step,
    ...
    gh, gw = patch_centers.shape[:2]
    ph, pw = patch_shape
    
    def patch_generator():
        for i in range(gh):
            for j in range(gw):
                center_h, center_w = patch_centers[i, j]
                
                # Calculate patch boundaries
                h_start = max(0, int(center_h - ph // 2))
                h_end = min(images.shape[-2], h_start + ph)
                h_start = max(0, h_end - ph)  # Ensure patch is exactly ph pixels
                
                w_start = max(0, int(center_w - pw // 2))
                w_end = min(images.shape[-1], w_start + pw)
                w_start = max(0, w_end - pw)  # Ensure patch is exactly pw pixels
                
                # Extract patch using slicing (creates a view, not copy)
                patch = images[..., h_start:h_end, w_start:w_end]
                yield patch
                
    return patch_generator(), patch_centers, (gh, gw)


def _patch_grid_generator_3d(
    images: torch.Tensor,
    patch_shape: tuple[int, int, int],
    patch_step: tuple[int, int, int],
    distribute_patches: bool = True,
) -> Tuple[Iterator[torch.Tensor], torch.Tensor, Tuple[int, int, int]]:
    """Generate 3D patches on-demand to save memory."""
    patch_centers = patch_grid_centers(
        image_shape=images.shape[-3:],
        patch_shape=patch_shape,
        patch_step=patch_step,
    ...
    gd, gh, gw = patch_centers.shape[:3]
    ...
    def patch_generator():
        for i in range(gd):
            for j in range(gh):
                for k in range(gw):
                    center_d, center_h, center_w = patch_centers[i, j, k]
                    
                    # Calculate patch boundaries
                    d_start = max(0, int(center_d - pd // 2))
                    d_end = min(images.shape[-3], d_start + pd)
                    ...
                    # For time series data with pd=1, we want 2D patches across all time frames
                    if pd == 1:
                        # This is a 2D patch extraction from time series data (t, h, w)
                        # Don't slice the first dimension (time), only spatial dimensions
                        patch = images[..., h_start:h_end, w_start:w_end]
                        # Add the singleton spatial dimension to match expected format
                        patch = patch.unsqueeze(-3)  # Shape: (t, 1, ph, pw)
                    else:
                        # This is true 3D patch extraction from 3D volume data
                        patch = images[..., d_start:d_end, h_start:h_end, w_start:w_end]
                    yield patch
                    
    return patch_generator(), patch_centers, (gd, gh, gw)
